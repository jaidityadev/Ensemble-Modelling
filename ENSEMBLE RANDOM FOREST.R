...
data_train <- data[Year %in% n_date[1:21]]
data_test <- data[Year %in% n_date[22]]

ggplot(data_train, aes(Year,Total)) +
  geom_point() + geom_smooth()
labs(x = "Year", y = "Number of People Affected by Dengue") +
  theme_ts

#Bagging 
data_ts <- ts(data_train$Total, freq = 12)
...
ggplot(preds_all, aes(Var2, value, color = as.factor(Var1))) +
  ...
  labs(x = "Year", y = "Number of People Affected by Dengue", title = "Comparison of Ensemble Learning forecasts") +
